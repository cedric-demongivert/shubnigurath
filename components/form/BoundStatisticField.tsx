 ...
 * 
 */
export class BoundStatisticField extends React.PureComponent<BoundStatisticField.Properties> {
  /**
   * 
   */
   ...
   * 
   */
  public constructor(properties: BoundStatisticField.Properties) {
    super(properties)
  ...
  public handleInputChange (event: DataEvent): void {
    this.props.onChange(DataEvent.change(this.props.value, DataReducer.reduce(this.props.value, event)))
  }
  ...
  public handlePancrement (event: IncrementEvent): void {
    ...
    const nextValue: number = clamp(this._origin + event.payload, 0, this.props.maximum)
   ...
   */
   public handleIncrement (event: DataEvent<number>): void {
    ...
    const nextValue: number = clamp(DataReducer.reduce(previousValue, event), 0, this.props.maximum)
      ...
      <Pancrement speed={1/40} onChange={this.handlePancrement}>
        <Field onClick={this.handleFieldClick} className='is-clickable'>
          ...
          <div className={classnames('control control-value', this.props.className)}>
            <Triplet superscript={ '/' + this.props.maximum }>
 ...
 * 
 */
export namespace BoundStatisticField {
  /**
   * 
   */
